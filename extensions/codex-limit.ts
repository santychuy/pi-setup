...
const WIDGET_KEY = "codex-limit-usage-widget";
const GLOBAL_USAGE_KEY = "piCodexLimit";
const FAST_STATUS_KEY = "codex-fast";
const FAST_MODELS = new Set(["gpt-5.4", "gpt-5.5", "gpt-5.6-luna", "gpt-5.6-sol", "gpt-5.6-terra"]);
...
function supportsFastMode(ctx: ExtensionContext): boolean {
  ...
  return model !== undefined && isOpenAICodexProvider(model.provider) && FAST_MODELS.has(model.id);
}
...
/** Update the widget visible above the editor. */
function publishWidget(ctx: ExtensionContext, snapshot: UsageSnapshot | undefined) {
  if (!ctx.hasUI) return;

  if (!isOpenAICodexProvider(ctx.model?.provider) || !snapshot) {
    ctx.ui.setWidget(WIDGET_KEY, undefined);
    return;
  }

  // Some accounts expose only the weekly window while the 5-hour limit is disabled.
  const usageWindow = snapshot.fiveHour ?? snapshot.weekly;
  ...
  ctx.ui.setWidget(WIDGET_KEY, (_tui, theme) => {
    ...
    const barSym = theme.fg("accent", "▰".repeat(filled) + "▱".repeat(barWidth - filled));
    const label = snapshot.fiveHour ? "Codex 5h" : "Codex 7d";
    const resetStr = formatResetLong(usageWindow?.resetAt);

    return {
      render(width): string[] {
        const usageLine = `${theme.fg("accent", label)} ${barSym} ${theme.fg(
          pct >= 90 ? "error" : pct >= 70 ? "warning" : "text",
          `${pct}%`,
        )} ${theme.fg("dim", resetStr)}`;
        return [truncateToWidth(usageLine, width, "")];
      },
      invalidate() {},
    };
  });
...
async function updateUsage(
  ...
  try {
    ...
    publishWidget(ctx, usageSnapshot);
...
export default function (pi: ExtensionAPI): void {
  ...
  const updateFastStatus = (ctx: ExtensionContext) => {
    if (!ctx.hasUI) return;
    ...
    if (!supportsFastMode(ctx)) {
      ctx.ui.setStatus(FAST_STATUS_KEY, ctx.ui.theme.fg("warning", "(unavailable)"));
      return;
    }
    ctx.ui.setStatus(
      FAST_STATUS_KEY,
      fastModeEnabled ? ctx.ui.theme.fg("accent", "(fast)") : undefined,
    );
  };

  pi.registerFlag("codex-fast", {
    description: "Start this session with Codex priority processing enabled",
    type: "boolean",
    default: false,
  });
    ...
    inFlight = inFlight
      ...
      .then(() =>
        currentGeneration === generation
          ? updateUsage(ctx, () => currentGeneration === generation)
          : undefined,
      );
    return inFlight;
  };
  ...
  pi.on("session_start", (_event, ctx) => {
    fastModeEnabled = pi.getFlag("codex-fast") === true;
  ...
  pi.on("before_provider_request", (event, ctx) => {
    if (!fastModeEnabled || !supportsFastMode(ctx)) return;
  ...
  pi.registerCommand("codex-fast", {
    description: "Toggle Codex priority processing for this session",
    ...
    handler: async (args, ctx) => {
      ...
      if (!(["", "on", "off", "status"] as string[]).includes(action)) {
        ctx.ui.notify("Usage: /codex-fast [on|off|status]", "info");
        return;
      }
      if (action === "") fastModeEnabled = !fastModeEnabled;
      else if (action !== "status") fastModeEnabled = action === "on";
      updateFastStatus(ctx);
      const state = fastModeEnabled ? "ON" : "OFF";
      ctx.ui.notify(`Codex Fast Mode: ${state} (Session only)`, "info");
    },
  });
